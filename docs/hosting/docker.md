# Self Hosting Finachy with Docker

This guide will help you setup, update, and maintain your self-hosted Finachy application with Docker Compose. Docker Compose is the most popular and recommended way to self-host the Finachy app.
...
```bash
# Download the sample compose.yml file from the Finachy Github repository
curl -o compose.yml https://raw.githubusercontent.com/finachium/finachy/main/compose.example.yml
```
...
If everything is working, you will see the Finachy login screen.
...
Most self-hosting users will want the Finachy app to run in the background on their computer so they can access it at all times. To do this, hit `Ctrl+C` to stop the running process, and then run the following command:
...
If you find bugs or have a feature request, be sure to read through our [contributing guide here](https://github.com/finachium/finachy/wiki/How-to-Contribute-Effectively-to-this-Project).
...
If you are trying to get Finachy started for the **first time** and run into database connection issues, it is likely because Docker has already initialized the Postgres database with a _different_ default role (usually from a previous attempt to start the app).

If you run into this issue, you can optionally **reset the database**.

**PLEASE NOTE: this will delete any existing data that you have in your Finachy database, so proceed with caution.**  For first-time users of the app just trying to get started, you're generally safe to run the commands below.

By running the commands below, you will delete your existing Finachy database and "reset" it.

```
docker compose down
docker volume rm finachy_postgres-data # this is the name of the volume the DB is mounted to
docker compose up
docker exec -it finachy-postgres-1 psql -U finachy -d finachy_production -c "SELECT 1;" # This will verify that the issue is fixed
```
